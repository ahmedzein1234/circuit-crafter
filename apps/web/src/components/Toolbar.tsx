...
export function Toolbar({ onHelpClick, onTemplatesClick }: ToolbarProps) {
  ...
  const handleExport = async () => {
    ...
    setIsExporting(true);
    try {
      await exportCircuitAsImage(stageRef, components, {
  ...
  return (
    <div className="flex items-center gap-1 md:gap-2">
      ...
      <button
        ...
        className={`hidden sm:flex items-center gap-1.5 px-3 py-2 min-h-touch-target md:min-h-11 text-mobile-xs md:text-sm font-medium rounded-lg transition-all duration-200 hover:scale-105 active:scale-95 focus:outline-none focus:ring-2 focus:ring-purple-500 ${
          exportSuccess
            ? 'bg-green-600 text-white'
            : 'bg-purple-600 hover:bg-purple-500 disabled:bg-gray-700 disabled:text-gray-500 text-white'
        }`}
        title="Export as image"
        aria-label="Export circuit as image"
      >
        {isExporting ? (
          <svg className="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24">
            <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
        ...
        <span className="hidden lg:inline">{exportSuccess ? 'Exported!' : 'Export'}</span>
      </button>
